...
// 사용자 정보 저장
function saveUserData(userId: string, uid: string, ltuid: string, ltoken: string, publicStatus: boolean = false): void {
    const encryptedLtoken = encrypt(ltoken);
    const savedLtoken = `${encryptedLtoken.iv}:${encryptedLtoken.authTag}:${encryptedLtoken.content}`;
    const existing = userData_db.prepare(`SELECT 1 FROM userData WHERE userId = ?`).get(userId);
    if (existing) {
        userData_db
            .prepare(
                `
...
// 사용자 공개 설정 변경
function setUserPublicStatus(userId: string, publicStatus: boolean = false): void {
    userData_db
        .prepare(
            `
        UPDATE userData
...
// 사용자 정보 조회
function getUserData(userId: string): { uid: string; ltuid: string; ltoken: string; public: boolean } | null {
    const row = userData_db.prepare("SELECT uid, ltuid, ltoken, public FROM userData WHERE userId = ?").get(userId) as
    ...
    return { uid: row.uid, ltuid: row.ltuid, ltoken: decryptedLtoken, public: row.public };
}
