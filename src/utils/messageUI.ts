...
export async function accountLinkUI(isTokenExpired: boolean = false) {
    ...
    const guideBTN = new ButtonBuilder().setCustomId("linkguide").setLabel("연동 가이드").setStyle(ButtonStyle.Success);
    const row = new ActionRowBuilder<ButtonBuilder>().addComponents(linkBTN, guideBTN);
    return { embed, row };
}

export async function gameRecordEmbed(gameRecord: any) {
    const expeditionPromises = gameRecord.expeditions.map(async (expedition: any) => {
        let emoji = await emojiFromUrl(expedition.icon);
...
export function playRecordEmbed(gameRecord: any) {
    const embed = new EmbedBuilder()
        ...
        .addFields(
            ...
            { name: "개방된 캐릭터", value: `${gameRecord.avatar_num}`, inline: true }
        );
    embed
        .addFields(
            {
            ...
            { name: "\u200B", value: "\u200B", inline: true },
            { name: "업적 달성 개수", value: `${gameRecord.achievement_num}`, inline: true }
        )
        .setTimestamp();
    return embed;
}
...
export async function achievementDetailEmbed(achievementDetail: any) {
    const embed = new EmbedBuilder()
        ...
        .addFields(
            { name: `${emojis.achive_gold}`, value: `${achievementDetail.gold_num}`, inline: true },
            { name: `${emojis.achive_silver}`, value: `${achievementDetail.silver_num}`, inline: true },
            { name: `${emojis.achive_cooper}`, value: `${achievementDetail.copper_num}`, inline: true }
        )
        .setTimestamp();

    const achievementPromises = achievementDetail.achievements.map(async (achievement: any) => {
    ...
    embed.addFields({ name: "총 업적 개수", value: `${achievementDetail.achievement_num}`, inline: false });
    return embed;
}
...
export function notExistEmbed() {
    const embed = new EmbedBuilder().setColor("Red").setDescription("조회하신 유저의 연동된 계정 정보가 없습니다.");
    return embed;
}

export function expiredEmbed() {
    const embed = new EmbedBuilder().setColor("Red").setDescription("조회하신 유저의 토큰이 만료되었습니다.");
    return embed;
}
    ...
    collector.on("collect", async (i: ButtonInteraction) => {
        if (i.user.id !== subjectId) {
            const embed = new EmbedBuilder()
                .setColor("Red")
                .setTitle("권한 없음")
                .setDescription(`계정 연동은 계정의 주체(<@${subjectId}>)만 사용할 수 있습니다.`);
            await i.reply({ embeds: [embed], ephemeral: true });
            return;
        }
        if (i.customId == "linkaccount") {
            const modal = new ModalBuilder().setCustomId("linkaccountmodal").setTitle("계정 연동 정보 입력");
            ...
            const uidLabel = new LabelBuilder().setLabel("UID").setTextInputComponent(uidInput);
            const ltuidLabel = new LabelBuilder().setLabel("LTUID").setTextInputComponent(ltuidInput);
            const ltokenLabel = new LabelBuilder().setLabel("LTOKEN").setTextInputComponent(ltokenInput);
