...
interface challengeRecord {
    peak: { current_progress: number; total_progress: number; rank_icon: string; rank_icon_type: string } | null;
    ...
    const processedUserData = {
        ...
        expeditions: liveNote.data.expeditions.map((expedition: any) => {
            return {
                ...
                finish_time: expedition.remaining_time != 0 ? dayjs(expedition.finish_ts * 1000).tz() : null,
            };
        }),
...
async function getAchievementDetail(uid: string, ltuid: string, ltoken: string) {
    const userInfo = await fetchUserInfo(uid, ltuid, ltoken);
...
async function getCharacterList(uid: string, ltuid: string, ltoken: string) {
    const characters_data = await fetchCharacters(uid, ltuid, ltoken).then((res) => res.data.avatar_list);
    if (!characters_data) {
        return null;
    }
    ...
    const characters = characters_data.map((char: any) => {
        ...
        if (char.servant_detail) {
            const servant_skills = char.servant_detail.servant_skills.map((skill: any) => {
                return {
                    remake: skill.remake,
                    icon: skill.item_url,
                    level: skill.level,
