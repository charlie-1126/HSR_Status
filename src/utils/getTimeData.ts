...
async function getTimeData() {
	...
	if (battleUpdateTime["currencyWar"].isSameOrBefore(now)) {
		battleUpdateTime["currencyWar"] = battleUpdateTime["currencyWar"].add(
			14,
			"day",
		);
	}
	...
	if (battleUpdateTime["simulation"].isSameOrBefore(now)) {
		battleUpdateTime["simulation"] = battleUpdateTime["simulation"].add(
			14,
			"day",
		);
	}
	...
	const challenges: Record<
		string,
		{ startTime: ReturnType<typeof dayjs>; endTime: ReturnType<typeof dayjs> }
	> = {};
	...
	if (offsetData.versionUpdate) {
		const offsetVersionUpdate = dayjs.tz(
	...
	if (offsetData.nextversionUpdate) {
		const offsetNextVersionUpdate = dayjs.tz(
	...
	if (offsetData.nextnextversionUpdate) {
		const offsetNextNextVersionUpdate = dayjs.tz(
	...
	if (
		warpTime.length > 0 &&
		warpTime[warpTime.length - 1].gameversion != gameversion
	) {
	...
	if (offsetData.previewProgramTime) {
		const offsetPreviewProgramTime = dayjs.tz(
		...
		if (now.isBefore(offsetPreviewProgramTime)) {
			...
			previewProgramTime = nextversionUpdate
				.subtract(12, "d")
				.set("hour", 20)
				.set("minute", 30)
				.set("second", 0);
	...
	if (now.isSameOrAfter(previewProgramTime) && nextnextversionUpdate != null) {
		previewProgramTime = nextnextversionUpdate
			.subtract(12, "d")
			.set("hour", 20)
			.set("minute", 30)
			.set("second", 0);
	...
	const weeklyResetTime = dailyResetTime.set(
		"date",
		dailyResetTime.date() + ((7 - dailyResetTime.day() + 1) % 7),
	);
	...
	return `## 대규모 업데이트\n- 버전 업데이트: <t:${data.versionUpdate.unix()}:R>\n- 프리뷰 스페셜 프로그램: <t:${data.previewProgramTime.unix()}:R>\n- 무명의 공훈 종료: ${
		...
		data.warpTime.length > 0
			? data.warpTime
					.map(
						(warp: any) =>
							`- ${warp.characters.join(", ")} 픽업 종료: <t:${warp.endTime.unix()}:R>`,
					)
					.join("\n")
			: "- 현재 진행중인 워프가 없습니다."
