...
const CLIENT_ID =
    (process.env.NODE_ENV === "development" ? process.env.TESTBOT_CLIENT_ID : process.env.CLIENT_ID) || "";
const EMOJI_DIR = path.join(__dirname, "../..", "emoji");
const EMOJI_URL_DIR = path.join(EMOJI_DIR, "url");

async function registerEmojis() {
    const rest = new REST().setToken(TOKEN);
    const uploadedEmojis = (await rest.get(`/applications/${CLIENT_ID}/emojis`)) as {
        items: APIEmoji[];
    };
    ...
    for (const emoji of urlEmojis) {
        const emojiName = emoji.split(".")[0];
        if (!uploadedEmojis.items.find((v) => v.name === emojiName)) {
            const base64 = fs.readFileSync(path.join(EMOJI_URL_DIR, emoji)).toString("base64");
            await rest.post(`/applications/${CLIENT_ID}/emojis`, {
                body: {
                    image: `data:image/${emoji.split(".")[1] === "png" ? "png" : "gif"};base64,${base64}`,
                    name: emojiName,
    ...
    const res = (await rest.get(`/applications/${CLIENT_ID}/emojis`)) as { items: APIEmoji[] };
...
async function emojiFromUrl(url: string): Promise<string> {
    ...
    logToFile("emojiManager", `Downloaded ${emojiName}.${fileExt} from ${url} to url folder`);
